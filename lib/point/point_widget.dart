...
class _PointWidgetState extends State<PointWidget> {
  ...
  Widget build(BuildContext context) {
    ...
    Future<http.Response> createPointRequest() async {
      final response = await http.post(
        Uri.parse('https://us-central1-gifticano-74f3e.cloudfunctions.net/gifticano/point'),
      ...
      if (((response.statusCode == 201) | (response.statusCode == 200)) & (jsonDecode(response.body)['result'] == "success")) {
        print('create point request succeeded');
      } else {
        print("안돼");
        await showDialog(
            context: context,
            builder: (alertDialogContext) {
              return AlertDialog(
                  title: Text('Error code: ${response.statusCode}'),
                  content: Text('해당 이미지를 캡쳐해서 문의해주세요.\n${jsonDecode(response.body)}'),
                  //예쁘게
                  ...
                  actions: [
                    TextButton(
                      onPressed: () => launchURL('http://pf.kakao.com/_yxaUSb/chat'),
                      child: Text('문의하기'),
                      //예쁘게
        ...
        throw Exception('Failed to create point request');
      }
    }
